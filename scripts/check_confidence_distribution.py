...
print("=== EEGNet Confidence Distribution (within-subject) ===\n")
all_confidences = []
...
print(f"  % trials > 0.74:  {(all_confidences>0.74).mean():.1%}")
print(f"  % trials > 0.80:  {(all_confidences>0.80).mean():.1%}")
print(f"\n=> Recommended threshold range: "
      f"wherever >60% coverage is achievable")
